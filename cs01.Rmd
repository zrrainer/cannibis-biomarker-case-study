...
OF = OF_ex |>
  ...
  ggplot(data = OF,
    ...
    geom_smooth(method = "lm", formula = y ~ x)
```
We fit 2 linear models on low dose group and high dose group respectivly. Quantifying the degree of change in correlation coefficients for each compound under different dosage conditions. This approach could provide insights into the magnitude of variations in compound interactions.
...
fit single variable linear model
1. value: thc, explanatory variable: cbg
```{r}
lm_cbg = linear_reg() |>
  set_engine("lm") |>
  fit(thc ~ cbg, data = OF) 
...
glance(lm_cbg)
```
here are resulting linear model is thc = 14.41cbg + 39.8 with an R^2 of 0.85 - which is already quite high. 
...
glance(mlm_cbg2)
```
Adding in treatment as another explanatory variable and including interaction effects produces a noticably more accurate model - with a R^2 of ~0.97. 

Just to be thorough, et's try another way. Here we fit a linear model for high does and low dose group *separately*. 
```{r}
OF_low <- OF |>
