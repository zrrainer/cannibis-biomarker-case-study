...
deleting compounds that obviously do not work from the compound data frame 
...
interesting. for the sake of our sanity, let's narrow our target of investigation to THC vs CBG in OF. 
...
for now lets do some EDA first.

### Extended Question: EDA
lets do a little gut check. since samples from different treatment yield different correlation between compounds, why does treatment not effect the actual distribution of the measuremnt of the compound...?
```{r}
ggplot(data = OF, mapping = aes(x = log(cbg))) +
  geom_histogram() +
  facet_wrap(~ treatment)

ggplot(data = OF, mapping = aes(x = log(thc))) +
  geom_histogram() +
  facet_wrap(~ treatment)
```
that *is* pretty weird. they have basically the same distribution. 

oh we can do a model of predicting one comp from other
and another model w one comp + treatment
